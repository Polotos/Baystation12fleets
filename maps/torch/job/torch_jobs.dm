...
/datum/map/torch/setup_job_lists()
	for(var/job_type in allowed_jobs)
		...
		if(job && (job.department_flag & COM | HCOM) && length(job.allowed_branches) && !(/datum/mil_branch/civilian in job.allowed_branches))
...
/datum/map/torch
	default_assistant_title = "Cerw in verification"
